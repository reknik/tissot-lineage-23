...
/*
 * Formats of the QSEED2 VIG pipes.  They are the same as plane_formats_yuv
 * except that the 10-bit P010 format is left out: these pipes only have the
 * 8-bit CSC block (no DPU_SSPP_CSC_10BIT), so 10-bit YUV cannot be
 * converted correctly.
 */
static const uint32_t plane_formats_yuv_qseed2[] = {
	DRM_FORMAT_ARGB8888,
	DRM_FORMAT_ABGR8888,
	DRM_FORMAT_RGBA8888,
	DRM_FORMAT_BGRX8888,
...
/*
 * QSEED2 scaler block.  It has no hardware version register, so
 * scaler_blk.version stays zero and the QSEED2 code path is selected by
 * DPU_SSPP_SCALER_QSEED2 instead.  VIG pipes are the video/image pipes and
 * support the full YUV format list.
 */
#define _VIG_SBLK_QSEED2() \
	{ \
	.scaler_blk = {.name = "scaler", \
		/* no version register on qseed2 */ \
		.base = 0x200, .len = 0xa0,}, \
	.csc_blk = {.name = "csc", \
		.base = 0x320, .len = 0x100,}, \
	.format_list = plane_formats_yuv_qseed2, \
	.num_formats = ARRAY_SIZE(plane_formats_yuv_qseed2), \
	.rotation_cfg = NULL, \
	}
